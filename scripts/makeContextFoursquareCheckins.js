...
pg.connect('postgres://eric@localhost/eric', function(err, client, cb) {
  ...
  client.query('select array_to_json(array_agg(row_to_json(t)))from (select superunit_id, checkinscount from site_totals where checkinscount > 0 order by checkinscount Desc) t', function(err, response) {
