...
pg.connect('postgres://eric@localhost/eric', function(err, client, cb) {
  ...
  client.query('select array_to_json(array_agg(row_to_json(t)))from (select su_id, flickrphotos from site_totals where flickrphotos > 0 order by flickrphotos Desc) t', function(err, response) {
