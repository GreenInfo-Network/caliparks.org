...
console.log('Making a JSON context file for Twitter...');

pg.connect('postgres://eric@localhost/eric', function(err, client, cb) {

  client.query('select array_to_json(array_agg(row_to_json(t)))from (select superunit_id, tweets from site_totals where tweets > 0 order by tweets Desc) t', function(err, response) {
