...
function saveInstagramHarvesterResults(photos, park) {
...
function getInstagramPhotosForCircles(circles, callback) {
  ...
  return async.each(circles, function(circle, done) {
    liveTaskCounter["circles"]++;
    ...
    q.push({
      ...
      fn: instagramRecursionQueueTask(null, circle.lat, circle.lng, circle.radius, null, circle.parks, 1, q)
    });
  ...
  return request({
    ...
    if (response.statusCode === 400 || response.statusCode === 429) {
      if (body.code === 420 || body.code === 429) {
        ...
        console.log("rate limited, sleeping %d seconds", sleeptime);

        // retry later
        return setTimeout(function() {
          return queryInstagramAPI(lat, lng, radius, callback);
        }, sleeptime * 1000);
      }

      return callback(new Error(JSON.stringify(response.body))); // Return body as err
    }
...
function instagramRecursionQueueTask(err, latMid, lngMid, radius, polygon, parks, depth, q) {
  return function(callback) {
    return queryInstagramAPI(latMid, lngMid, radius, function(err, photos) {
      if (err) {
        console.warn(err.stack);
        return callback(err);
      }
      ...
      console.log("got %d instagram photos for (%d, %d) (%dm) at depth %d", count, latMid, lngMid, radius, depth);

      // If the count is 100, I think I need to recurse here.

      saveInstagramHarvesterMetadata(null, latMid, lngMid, radius, new Date(), count);
      saveInstagramHarvesterResults(photos, null);

      if (count >= 100 && radius > 10) {    // TODO improve this. For now, stop if radius drops below X metres!
        console.log("TOO MANY RESULTS; splitting");

        // don't return; this just happens sometimes
        pg.connect(DATABASE_URL, function(err, client, done) {
          ...
          // TODO use the instagram_metadata table as a queue instead, seeding
          // that in the same way as instagram_array
          var query = [
            ...
            "    geom AS the_geom",
            "  FROM (",
            util.format("    SELECT GetIntersectingHexagons(ST_Buffer(ST_Transform('SRID=4326;POINT(%d %d)'::geometry, 3310), $1), $2) geom", lngMid, latMid),
            "  ) AS _",
            // prevent duplicate key errors
            "  WHERE (SELECT COUNT(lat) FROM instagram_array WHERE the_geom = geom LIMIT 1) = 0"
          ].join("\n");

          console.log(query);
          console.log([radius, radius / 5]);

          return client.query(query, [radius, radius / 5], function(err) {
            if (err) {
              console.warn(err.stack);
              return;
            }

            console.log("New hexes created.");
            process.exit();
            // TODO queue up these centers
          });
        });
        ...
        */
        ...
        //       console.log("circle tasks:", liveTaskCounter["circles"]);
        //       q.push({
        //         name: 'another task, depth ' + nextDepth,
        //         centerX: newX,
        //         centerY: newY,
        //         radius: newRadius,
        //         fn: instagramRecursionQueueTask(null, newY, newX, newRadius, polygon, result, nextDepth, q)
        //       });
        //     }
        //   });
        // });
    ...
    return pg.connect(DATABASE_URL, function(err, client, done) {
      ...
      var query = client.query.bind(client),
          select = [
              ...
              "ORDER BY last_fetched ASC, radius DESC",
              "LIMIT $1"
            ].join("\n"),
...
var harvest = function(callback) {
  callback = callback || NOOP;

  var balancer = new Balancer();

  for (var i = 0; i < 10; i++) {
    var worker = new Worker(function(task, done) {
      // console.log("%d:", i, task);
      ...
      return queryInstagramAPI(task.lat, task.lng, task.radius, function(err, photos) {
        ...
        // TODO transactions
        // beginTransaction(function(err, commit, rollback) {
        // });

        // TODO update metadata (count (as ST_DWithin), last_fetched, locked)
        ...
        if (photos.length >= 100 && task.radius > 10) {
          ...
          // not returning, as this is optional
          pg.connect(DATABASE_URL, function(err, client, done) {
            if (err) {
              console.warn(err.stack);
              return;
            ...
            // TODO delete parent hex to avoid subsequent duplicate queries
            ...
            var query = [
                  ...
                  "    geom",
                  "  FROM (",
                  "    SELECT GetIntersectingHexagons(ST_Buffer(ST_Transform(ST_SetSRID(ST_MakePoint($1, $2), 4326), 3310), $3), $4) geom",
                  "  ) AS _",
                  // prevent duplicate key errors
                  "  WHERE (SELECT COUNT(id) FROM instagram_regions i WHERE i.geom = _.geom LIMIT 1) = 0"
                ].join("\n"),
                params = [task.lng, task.lat, task.radius, task.radius / 5];

            return client.query(query, params, function(err) {
              if (err) {
                console.warn(err.stack);
                return;
              }
    ...
    balancer.push(worker);
  }

  createSource().pipe(balancer);
};
