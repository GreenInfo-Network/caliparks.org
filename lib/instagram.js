...
function getInstagramPhotosForCircles(circles, callback) {
  ...
  return async.each(circles, function(circle, done) {
    ...
    // TODO this is just treating the queue as a funnel
    q.push({
      name: "",
      centerX: circle.lng,
      centerY: circle.lat,
      radius: circle.radius
    }, instagramRecursionQueueTask(null, circle.lat, circle.lng, circle.radius, null, circle.parks, 1, q, callback));
...
function instagramRecursionQueueTask(err, latMid, lngMid, radius, polygon, parks, depth, q, callback) {
