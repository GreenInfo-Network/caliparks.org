...
var split = function(region, callback) {
  ...
  return pg.connect(DATABASE_URL, function(err, client, done) {
    ...
    // this makes sense for bulk loads, but not necessary refreshes
    // TODO mark parent as split
    ...
    var query = [
          ...
          // TODO
          // "  WHERE (SELECT COUNT(id) FROM instagram_regions i WHERE i.geom = _.geom) = 0"
          // avoid boxiness
          "  WHERE ST_Intersects(geom, (SELECT geom FROM instagram_regions WHERE id = $1))"
        ].join("\n"),
        params = [region.id, region.lng, region.lat, region.radius, region.radius / 5];

    return client.query(query, params, function() {
      done();
