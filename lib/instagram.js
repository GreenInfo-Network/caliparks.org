...
function queryInstagramAPI(region, callback) {
  console.log("[*] querying Instagram with for region %d with center (%d, %d) and radius %d", region.id, region.lat.toFixed(4), region.lng.toFixed(4), region.radius);

  var since = region.lastFetched;

  if ((since = region.lastFetched)) {
    since = since.getTime();
  } else {
    var d = new Date();
  ...
  return operation.attempt(function(currentAttempt) {
    return request({
      ...
      if (response.statusCode === 500) {
        return callback(new Error("Instagram error: code " + response.statusCode + " " + response.body));
      }
  ...
  return _(function(push, next) {
    return pg.connect(DATABASE_URL, function(err, client, done) {
      ...
      var query = client.query.bind(client),
          select = [
              "SELECT id, ST_X(center) AS lng, ST_Y(center) AS lat, radius, last_fetched AS lastFetched",
