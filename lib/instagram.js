...
function queryInstagramAPI(region, each, done) {
  console.log("[*] Instagram / region %d (%d, %d) @%dm [%s]", region.id, region.lat.toFixed(4), region.lng.toFixed(4), region.radius, region.last_fetched);
    ...
    return pg.connect(DATABASE_URL, function(err, client, done) {
      ...
      var query = client.query.bind(client),
          select = [
              ...
              "ORDER BY radius DESC, COALESCE(last_fetched, '2014-10-04'::timestamp with time zone) ASC",
              "LIMIT $1"
            ].join("\n"),
