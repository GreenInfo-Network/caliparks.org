    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          'LEFT JOIN (SELECT su_id, geom FROM cpad_entry_points LIMIT 1) AS cpad_entry_points ON cpad_entry_points.su_id = shortlist.superunit_id',
...
var getPark = function getPark(parkId, options, callback) {
  return pg.connect(DATABASE_URL, function(err, client, done) {
    ...
    var query = [
      ...
      "LEFT JOIN (SELECT su_id, geom FROM cpad_entry_points LIMIT 1) AS cpad_entry_points ON cpad_entry_points.su_id = cpad_superunits.superunit_id",
      "LEFT JOIN activities USING (superunit_id)",
