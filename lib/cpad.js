...
var buildQuery = function buildQuery(dbQuery, data, callback) {
  // Split activities into an array and limit the total count
  var queryArray = sanitizer.sanitize((data.query.with || '')).replace(/%20/," ").split(',').slice(0, 20),
  ...
  if (data.query.bbox) {
    ...
    fullQuery = {
      text: escape([
        ...
        '  AND activities @> %s',
        '  AND (shortlist.unit_name || array_to_string(activities.activities, \' \')) ILIKE %L',
  ...
  } else if (data.query.with && data.query.with.length && data.query.near && data.query.near.length) { //Has amenities & near filter
    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          '  pow((5000/GREATEST(geo.distance,1)),3)*stats.score as geoscore',
          ...
          'LEFT JOIN (SELECT su_id, geom FROM cpad_entry_points WHERE pt_type = \'primary\' LIMIT 1) AS cpad_entry_points ON cpad_entry_points.su_id = shortlist.superunit_id',
          'WHERE (shortlist.unit_name || array_to_string(shortlist.activities, \' \')) ILIKE %L',
          // '  AND shortlist.gis_acres >= 50',
          "  AND ARRAY['kiteboardingWindsurfing','climbing','horsebackRiding','rusticCabins','caving','whitewaterRaftingKayaking','boating','backpacking','historicalSite','fishing','biking','hiking','swimming','ohv','kayakingCanoeing','snowSports','wildlifeWatching','surfing','camping'] @> shortlist.activities",
          // 'ORDER BY geoscore DESC',
          // 'ORDER BY score DESC',
          'ORDER BY geo.distance ASC',
          'OFFSET $4',
          'LIMIT $3'
        ].join('\n'), '%'+dbQuery+'%'),
    ...
    fullQuery = {
      text: escape([
        ...
        'OFFSET $1',
        'LIMIT $2'
    ].join('\n'), scoreSubQuery, 'ARRAY[' + activities + ']::text[]', '%' + dbQuery + '%', scoreOrderBy),
      values : [startat, perpage]
    };
    ...
    fullQuery = {
      text: escape([
        ...
        'LEFT JOIN (select *, activities::text as activitext from activities) activities USING (superunit_id)',
        'LEFT JOIN (SELECT su_id, geom FROM cpad_entry_points WHERE pt_type = \'primary\' LIMIT 1) AS cpad_entry_points ON cpad_entry_points.su_id = shortlist.superunit_id',
        'WHERE (shortlist.unit_name || array_to_string(activities.activities, \' \')) ILIKE %L',
        '  AND access_typ=\'Open Access\'',
        'ORDER BY %s shortlist.unit_name ASC',
        'OFFSET $1',
        'LIMIT $2'
...
var dbLimit = '',
  ...
  return pg.connect(env.require('DATABASE_URL'), function(err, client, done) {
    ...
    buildQuery(dbQuery, options, function(err, fullQuery, bounds) {
