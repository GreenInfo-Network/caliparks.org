    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          'SELECT',
          'shortlist.unit_name,',
          'ST_AsGeoJSON(shortlist.geom) AS geometry,',
          'ST_AsGeoJSON(ST_Centroid(shortlist.geom)) AS centroid,',
          'activities,',
          'geo.distance distance,',
          'pow((5000/geo.distance),3)*stats.score as geoscore',
          'FROM (  SELECT    *  FROM cpad_superunits_4326  LEFT JOIN activities USING (superunit_id)  WHERE ST_DWithin(geom, ST_SetSRID(ST_MakePoint($1,$2), 4326), 2.7)) AS shortlist',
          ...
          'LEFT JOIN (SELECT *, ST_Distance(cpad_superunits_4326.geom::geography, ST_SetSRID(ST_MakePoint($1,$2), 4326)::geography) as distance FROM cpad_superunits_4326) geo ON geo.superunit_id = shortlist.superunit_id',
