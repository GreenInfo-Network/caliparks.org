...
var buildQuery = function buildQuery(dbQuery, data, callback) {
  ...
  if (data.query.with && data.query.with.length && data.query.near && data.query.near.length) { //Has amenities & near filter
    ...
    //
    queryArray = sanitizer.sanitize(data.query.with.toLowerCase()).split('+');
    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          'WHERE (shortlist.unit_name||shortlist.activities::text) ILIKE %L ',
          'AND activities @> %s',
          'ORDER BY geoscore DESC',
          'OFFSET $4',
          'LIMIT $3'
        ].join('\n'), '%' + dbQuery + '%', 'ARRAY' + JSON.stringify(activities.toLowerCase().split(',')).replace(/'/g, '').replace(/\"/g, "'")),
    ...
    fullQuery = {
      text: escape([
        ...
        '  ST_AsGeoJSON(COALESCE(cpad_entry_points.geom, ST_Centroid(ST_Transform(shortlist.geom, 4326)))) AS centroid',
        '  %s',
        'FROM (select *, activities::text as activitext from activities) activities',
        'JOIN cpad_superunits shortlist USING (superunit_id)',
        'LEFT JOIN (SELECT su_id, geom FROM cpad_entry_points WHERE pt_type = \'primary\' LIMIT 1) AS cpad_entry_points ON cpad_entry_points.su_id = shortlist.superunit_id',
        'WHERE activities @> %s',
        '  AND (shortlist.unit_name||activities.activities::text) ILIKE %L',
        '  AND access_typ=\'Open Access\'',
        'ORDER BY %s shortlist.unit_name ASC',
        'OFFSET $1',
        'LIMIT $2'
    ].join('\n'), scoreSubQuery, 'ARRAY' + JSON.stringify(activities.toLowerCase().split(',')).replace(/'/g, '').replace(/\"/g, "'"), '%' + dbQuery + '%', scoreOrderBy),
