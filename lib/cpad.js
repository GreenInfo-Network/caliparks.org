...
var buildQuery = function buildQuery(dbQuery, data, callback) {
  ...
  var scoreSubQuery = [
    ...
    '  ),',
    '  hipcamp AS (',
    '    SELECT su_id AS superunit_id, (activities ->> \'activityCount\')::integer AS activities FROM site_hipcamp_activities',
    '  ),',
    '  cpad_facilities AS (',
    '    SELECT superunit_id, COUNT(DISTINCT type) AS count FROM cpad_facilities GROUP BY superunit_id',
    '  )',
    '  SELECT (stats.flickr_photo_count + stats.instagram_photo_count + stats.tweet_count + stats.foursquare_venue_count + stats.foursquare_tip_count) * (CASE WHEN COALESCE(hipcamp.activities, 0) > 0 THEN 10 ELSE 1 END) * (CASE WHEN COALESCE(cpad_facilities.count, 0) > 0 THEN 5 ELSE 1 END) AS score',
    '  FROM stats',
    '  FULL OUTER JOIN hipcamp USING (superunit_id)',
    '  FULL OUTER JOIN cpad_facilities USING (superunit_id)',
