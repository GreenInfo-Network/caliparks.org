    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          'ST_AsGeoJSON(ST_Centroid(ST_Transform(shortlist.geom, 4326))) AS centroid,',
          'activities,',
...
var getPark = function getPark(parkId, options, callback) {
  return pg.connect(DATABASE_URL, function(err, client, done) {
    ...
    var query = [
      "SELECT",
      "  superunit_id, unit_name, manager_id, mng_agncy, access_typ, gis_acres, park_url,",
      "  ST_AsGeoJSON(ST_Envelope(ST_Transform(geom, 4326))) AS bbox,",
