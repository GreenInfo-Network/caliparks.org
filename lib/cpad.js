...
var buildQuery = function buildQuery(dbQuery, data, callback) {
  ...
  var queryArray = sanitizer.sanitize((data.query.with || '').toLowerCase()).split('+').slice(0, 20),
    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          'OFFSET $4',
          'LIMIT $3'
        ].join('\n'), '%' + dbQuery + '%', 'ARRAY[' + activities + ']::text[]'),
        values: [place.coordinates[1], place.coordinates[0], perpage, startat]
      };

      return callback(null, fullQuery, place.viewport);
    });
...
//cpad_superunits
var getParksByIdList = function getParksByIdList(idListArray, callback) {
  return pg.connect(DATABASE_URL, function(err, client, done) {
    if (err) {
      done();
      return callback(err);
