  ...
  if (data.query.with && data.query.with.length && data.query.near && data.query.near.length) { //Has amenities & near filter
    ...
    return getPlace(data.query.near, function(err, place) {
      ...
      fullQuery = {
        text: escape([
          ...
          'OFFSET $4',
          'LIMIT $3'
        ].join('\n'), '%' + dbQuery + '%', 'ARRAY' + JSON.stringify(activities.split(',')).replace(/'/g, '').replace(/\"/g, "'")),
    ...
    fullQuery = {
      text: escape([
        ...
        'OFFSET $1',
        'LIMIT $2'
    ].join('\n'), scoreSubQuery, 'ARRAY' + JSON.stringify(activities.split(',')).replace(/'/g, '').replace(/\"/g, "'"), '%' + dbQuery + '%', scoreOrderBy),
