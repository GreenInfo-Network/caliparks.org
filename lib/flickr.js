...
var getPhotosForRegion = function(region, page, photos, callback) {
  ...
  return request({
    ...
    if (rsp.statusCode === 200) {
      ...
      if (hasMorePages(body) && page <= body.photos.page && page < 100) {      //Stop after 100. TODO: do this better; geo searches return an approximate number of images
        console.log("[*] found another page for region %d (%s): %d of %d", region.id, bbox, body.photos.page, body.photos.pages);
        return getPhotosForRegion(region, page + 1, photos, callback);
      }

      return callback(null, photos, page);  // callback(err, result)
    }

    return callback(new Error(util.format("Flickr error %d:", rsp.statusCode, body)));
  });
};
...
var createWorker = function() {
  return new Worker(function(task, done) {
    return getPhotosForRegion(task, function(err, photos) {
      if (err) {
        // unlock
