...
var pgToGeoJSON = require('./pg-to-geojson.js');
...
dataFormatResponders['.geojson'] = function dataFormatResponderJSON(res, data, format, whitelist) {
  ...
  var featureCollection = new pgToGeoJSON.GeoFeatureCollection(dataOut.parks, {
    excludeProperties: ["geometry"]
  });

  featureCollection.properties = featureCollection.properties || {};
  // HACK none of the template data was previously being passed through; we
  // know we want place, so just pass that
  featureCollection.properties.place = data.place;
