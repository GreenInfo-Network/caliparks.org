...
flickrParkTable:
	psql -U openspaces -h geo.local -c "drop table flickr_photos_distinct;" \
	&& psql -U openspaces -h geo.local -c "create table flickr_photos_distinct as select distinct on (photoid) * from flickr_photos;" \
	&& psql -U openspaces -h geo.local -c "drop table park_flickr_photos_temp;" \
...
instagramParkTable:
	...
	&& psql -U openspaces -h geo.local -c "drop table park_instagram_photos_temp;" \
